#
# This script produces layers that indicate forest loss in the Mapbiomas transitions layers. The transition layers have a spatial filter applied to them (see documentation for details).
...
forest_loss_transitions <- sort(c(
    300 + classes$value[!(classes$value %in% c(3,6))],
    600 + classes$value[!(classes$value %in% c(3,6))]
    ))
...
ParGetForestGain <- function(r_path) {
    ...
    r_gain <- r %in% forest_loss_transitions

    output_filename <- paste0("forest_loss_transitions_", substr(r_name, nchar(r_name)-3, nchar(r_name)), ".tif")

    r_gain |> terra::writeRaster(filename = paste0(project_path, "data/constructed/raster/mapbiomas-transitions/forest-loss-transitions/", output_filename), overwrite = TRUE)
