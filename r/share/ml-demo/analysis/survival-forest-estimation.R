...
# These are handled inside the wrapper function.
f_list$baseline       <- c("mb_char", "mb_sum15_forest", "mb_sum15_nonforest", paste0("mb_lag_", 1:15))

# The outcome variable will be based on this variable
# Handled inside the wrapper function.
f_list$outcome        <- c("mb_forest")

within_elements <- names(f_list)

results_list <- list()

results_list$ref <- wrapRanger(dt_path = filenames$ml_demo$master_dt_edited, esample_path = filenames$ml_demo$esample_ranger_prepared, type = "ref", min_slen = 1, n_sample = 5000, features_list = f_list, within_elements = NULL)
results_list$def <- wrapRanger(dt_path = filenames$ml_demo$master_dt_edited, esample_path = filenames$ml_demo$esample_ranger_prepared, type = "def", min_slen = 1, n_sample = 5000, features_list = f_list, within_elements = NULL)

results_list$ref_min3 <- wrapRanger(dt_path = filenames$ml_demo$master_dt_edited, esample_path = filenames$ml_demo$esample_ranger_prepared, type = "ref", min_slen = 3, n_sample = 5000, features_list = f_list, within_elements = NULL)
results_list$def_min3 <- wrapRanger(dt_path = filenames$ml_demo$master_dt_edited, esample_path = filenames$ml_demo$esample_ranger_prepared, type = "def", min_slen = 3, n_sample = 5000, features_list = f_list, within_elements = NULL)

results_list$ref_within <- wrapRanger(dt_path = filenames$ml_demo$master_dt_edited, esample_path = filenames$ml_demo$esample_ranger_prepared, type = "ref", min_slen = 1, n_sample = 5000, features_list = f_list, within_elements = within_elements)
results_list$def_within <- wrapRanger(dt_path = filenames$ml_demo$master_dt_edited, esample_path = filenames$ml_demo$esample_ranger_prepared, type = "def", min_slen = 1, n_sample = 5000, features_list = f_list, within_elements = within_elements)
